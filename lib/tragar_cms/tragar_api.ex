defmodule TragarCms.TragarApi do
  ...
  defp accept_quote_with_token(token, quote_obj, acceptance_type, accepted_by, opts) do
    ...
    # Add waybillNumber if provided
    query_params =
      case Keyword.get(opts, :waybill_number) do
        nil -> query_params
        waybill_number -> Keyword.put(query_params, :waybillNumber, waybill_number)
      end
  ...
  defp parse_freightware_quotes(response_body, opts) do
    ...
    case response_body do
      ...
      # Fallback for legacy esWaybills format
  ...
  defp convert_freightware_quote_to_cms_format(quote) when is_map(quote) do
    %{
      ...
      "consigneedefmodule TragarCms.TragarApi do
  @moduledoc """
  Client for interacting with the FreightWare/Tragar API to fetch quotes.
  Handles authentication and quote retrieval operations.
  """
  ...
  @doc """
  Gets a quick quote (rate estimation) without creating an official quote.
  """
  def quick_quote(shipment_data, opts \\ []) do
    with {:ok, token} <- authenticate(opts),
         {:ok, response} <- quick_quote_with_token(token, shipment_data) do
  ...
  defp get_quote_with_token(token, quote_identifier) do
    ...
    case Req.get(url,
      ...
      {:ok, %{status: 200, body: response_body}} when is_map(response_body) ->
        Logger.info("Successfully fetched quote #{quote_identifier} from FreightWare")
        quote = parse_single_quote_response(response_body)
        {:ok, quote}
  ...
  defp update_quote_with_token(token, quote_identifier, quote_data) do
    ...
    case Req.put(url,
      ...
      {:ok, %{status: 200, body: response}} ->
        Logger.info("Successfully updated quote #{quote_identifier} in FreightWare")
        {:ok, response}
  ...
  defp quick_quote_with_token(token, shipment_data) do
    ...
    case Req.post("#{@base_url}/FreightWare/V1/quotes/quick",
      ...
      {:ok, %{status: 200, body: response}} ->
        Logger.info("Successfully retrieved quick quote from FreightWare")
        {:ok, response}
  ...
  defp parse_freightware_quotes(response_body, opts) do
    ...
    case response_body do
      # Handle esQuotes response (correct API spec format)
      %{"response" => %{"esQuotes" => quotes_data}} when is_binary(quotes_data) ->
        case Jason.decode(quotes_data) do
          {:ok, parsed_quotes} ->
            parsed_quotes
  ...
  defp parse_single_quote_response(response_body) do
    case response_body do
      %{"response" => %{"esQuotes" => quote_data}} when is_binary(quote_data) ->
        case Jason.decode(quote_data) do
          {:ok, parsed_quote} ->
            ...
            |> extract_quotes_from_response()
            |> List.first()
            |> convert_freightware_quote_to_cms_format()

          {:error, _} ->
            Logger.warning("Failed to parse FreightWare quote JSON response")
            generate_sample_quotes(1) |> List.first()
        end
      ...
      _ ->
        Logger.warning("Unexpected FreightWare single quote response format: #{inspect(response_body)}")
        generate_sample_quotes(1) |> List.first()
    end
  end
